...
class CreateCustomerViewTestCase(TestCase):
        ...
        self.customer_data.update(
            {
                "user_id": User.objects.create_user(
                    **get_user_data(username=random_user)
                ).id
            }
        )
    ...
    def test_create_customer_view_post_request_with_valid_data(self):
        ...
        response = self.client.post(CREATE_CUSTOMER_URL, data=data)
        self.assertEqual(response.status_code, 302)
    ...
    def test_create_customer_view_post_request_with_invalid_data(self):
        ...
        user = User.objects.create_user(**get_user_data(random_username()))
        self.customer_data.update(
            {"email": "invalid-email", "schedule": "invalid-schedule"}
        )
        response = self.client.post(CREATE_CUSTOMER_URL, data=self.customer_data)
        self.assertEqual(response.status_code, 200)
        ...
        self.assertFormError(
            ...
            "form",
            "schedule",
            "Select a valid choice. invalid-schedule is not one of the available choices.",
        )
...
class UpdateCustomerViewTestCase(TestCase):
    ...
    def test_update_customer_view_correct_template(self):
        ...
        self.assertTemplateUsed(response, expected_template)
        self.assertEqual(force_str(content.strip()), expected_template_content.strip())
