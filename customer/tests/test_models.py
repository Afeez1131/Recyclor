...
class CustomerModelTestCase(TestCase):
    ...
    def test_company_get_last_service_property(self):
        ...
        customer.services.create(
            cost=Decimal(5000),
            created_by=User.objects.create_user(**get_user_data())
        )
        data = get_user_data()
        data.update({"username": "testuser2"})
        customer.services.create(
            cost=Decimal(12000),
            created_by=User.objects.create_user(**data)
        )
    ...
    def test_company_property_get_next_service_date(self):
        ...
        self.assertEqual(customer.get_next_service_date, timezone.now().date() + timezone.timedelta(weeks=1))
...
class CustomerServiceHistoryTestCase(TestCase):
    ...
    def test_service_history_string_repr(self):
        ...
        self.assertEqual(str(history), f"{self.customer.name} - {history.created.date()} - {history.cost}")
